...
- 输出目录固定为 `成品_A轻盈时装杂志_保色修正版` 与 `PSD可编辑_A轻盈时装杂志_保色修正版`。
- 不增加白色云雾、曝光、曲线或调色覆盖层；排版只叠加文字、酒红细线与编号。
...
每张图建立 `原图_锁定`、`主标题_上`、`主标题_下`、`副标题`、`英文栏目标`、`页码/编号竖条` 与 `编辑细线` 图层，文字层不栅格化，不建立任何影响源图亮度与颜色的覆盖层。

- [x] **Step 3: 实现保存与导出**

PSD 保存到 `PSD可编辑_A轻盈时装杂志_保色修正版`，PNG 保存到 `成品_A轻盈时装杂志_保色修正版`，源文件与历史版本均不覆盖。
...
**Files:**
- Inspect: `E:\Studio-Assets\周周的内衣内裤项目\详情图\成品_A轻盈时装杂志_保色修正版\*.png`
- Inspect: `E:\Studio-Assets\周周的内衣内裤项目\详情图\PSD可编辑_A轻盈时装杂志_保色修正版\*.psd`
...
- Joe 反馈二版亮度过曝、浅绿色被洗淡。定位到 JSX 中 76%–80% 暖白模糊层为额外增亮源，而不是源图本身。
- 已删除四处渐隐层调用及对应函数，在独立的“保色修正版”目录重新生成 8 张 PNG 与 8 张 PSD，PNG 全部为 1776×2368。
- 新版与源图逐像素复核：商品边缘区域新增亮度超过 12 的像素占比 8/8 均不高于 0.001%，最大值为第 05 张的 2 个抗锯齿像素（0.000375%）；未再对商品做面积性增亮或调色。
- 已制作并检查 8 张联系表；另以原尺寸抽查左编辑版 01、右编辑版 02、竖条编号版 03 和 06。
- 已在 Photoshop 打开保色修正版 `03_柔软贴肤.psd`，确认上下标题、副标题、英文栏目标、细线与编号均为独立可编辑图层。
- 旧版输出保留在原目录，保色修正版使用独立目录，不覆盖源素材或历史版本。
